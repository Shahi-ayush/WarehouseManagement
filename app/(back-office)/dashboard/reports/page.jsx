...
export default function Reports() {
  ...
  const [mostSoldDaily, setMostSoldDaily] = useState([]);
  const [mostSoldMonthly, setMostSoldMonthly] = useState([]);
  const [forecastLowStock, setForecastLowStock] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  // 🔹 Stock Status
  const outOfStockItems = items.filter(item => item.quantity === 0);
  const lowStockItems = items.filter(item => item.quantity > 0 && item.quantity <= (item.reOrderPoint || 5));
  const highStockItems = items.filter(item => item.quantity > (item.reOrderPoint || 5) * 2);
  ...
  // 🔹 Forecasted Low Stock (Sliding Window Average)
  useEffect(() => {
    if (!items.length || !salesData.length) return;

    const daysWindow = 7; // last 7 days
    const warningDays = 3; // alert if predicted to run out in <= 3 days
    const today = new Date();

    const forecast = items.map(item => {
      // Filter last N days sales for this item
      const recentSales = salesData.flatMap(sale => sale.items)
        .filter(saleItem => saleItem.id === item.id)
        .filter(saleItem => {
          const saleDate = new Date(saleItem.saleDate || sale.createdAt);
          return saleDate >= new Date(today - daysWindow * 24 * 60 * 60 * 1000);
        });

      const totalSold = recentSales.reduce((sum, s) => sum + s.qty, 0);
      const avgDailySale = totalSold / daysWindow;
      const daysLeft = avgDailySale > 0 ? item.quantity / avgDailySale : Infinity;

      return { ...item, avgDailySale, daysLeft };
    }).filter(item => item.daysLeft <= warningDays && item.quantity > 0);

    setForecastLowStock(forecast);
  }, [items, salesData]);
  ...
  const baseActivities = [
    ...items.map(i => ({ id: i.id, type: "Item Created", message: `Item "${i.title}" was created.`, createdAt: i.createdAt })),
    ...categories.map(c => ({ id: c.id, type: "Category Created", message: `Category "${c.title}" was created.`, createdAt: c.createdAt })),
    ...brands.map(b => ({ id: b.id, type: "Brand Created", message: `Brand "${b.title}" was created.`, createdAt: b.createdAt })),
    ...suppliers.map(s => ({ id: s.id, type: "Supplier Created", message: `Supplier "${s.title}" was added.`, createdAt: s.createdAt }))
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Forecasted Low Stock */}
      <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
        <h2 className="font-semibold mb-4">Forecasted Low Stock</h2>
        {forecastLowStock.length === 0 ? (
          <p className="text-gray-500">No items forecasted to run out soon</p>
        ) : (
          <ul className="space-y-2 max-h-64 overflow-y-auto">
            {forecastLowStock.map(item => (
              <li key={item.id} className="flex justify-between border-b py-1 text-sm">
                <span>{item.title}</span>
                <span>{item.quantity} left (~{item.daysLeft.toFixed(1)} days)</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
          <h2 className="font-semibold mb-4">Top Sold Items Today</h2>
          {mostSoldDaily.length === 0 ? <p className="text-gray-500">No sales today</p> :
            <ul>
              {mostSoldDaily.map(item => (
                <li key={item.name} className="flex justify-between py-1 border-b">
                  <span>{item.name}</span>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm">
          <h2 className="font-semibold mb-4">Top Sold Items This Month</h2>
          {mostSoldMonthly.length === 0 ? <p className="text-gray-500">No sales this month</p> :
            <ul>
              {mostSoldMonthly.map(item => (
                <li key={item.name} className="flex justify-between py-1 border-b">
                  <span>{item.name}</span>
