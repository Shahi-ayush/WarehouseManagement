...
//   };
//   return (
//     <span className={`text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${styles[variant] || "bg-gray-100 text-gray-600"}`}>
//       {text}
//     </span>
//   );
// };
...
//   <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
//     {title && <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-3">{title}</p>}
//     {children}
//   </div>
// );

// const BarChart = ({ purchases }) => {
//   const monthly = purchases.reduce((acc, p) => {
//     const month = new Date(p.createdAt).toLocaleString("default", { month: "short" });
//     acc[month] = (acc[month] || 0) + p.total;
//     return acc;
...
//   if (!entries.length) return <p className="text-sm text-gray-400 py-4 text-center">No purchase data yet</p>;
...
//             title={`NPR ${total.toLocaleString()}`}
//             style={{ height: `${Math.round((total / max) * 72)}px` }}
//             className={`w-full rounded-t transition-all ${i === entries.length - 1 ? "bg-blue-500 dark:bg-blue-400" : "bg-blue-200 dark:bg-blue-800"}`}
//           />
//           <span className="text-[10px] text-gray-400">{month}</span>
//         </div>
//       ))}
...
//   if (loading)
//     return <div className="flex items-center justify-center min-h-screen text-sm text-gray-400">Loading dashboard...</div>;
//   if (!profile) return null;
...
//           </div>
//           <button onClick={logout} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-red-50 hover:text-red-600 hover:border-red-200 dark:hover:bg-red-950 dark:hover:text-red-400 transition-colors">
//             Logout
//           </button>
//         </div>
//       </header>
...
//           <div className="flex items-center justify-between px-4 py-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-xl">
//             <p className="text-sm text-red-700 dark:text-red-300">
//               Outstanding balance of <span className="font-medium">NPR {dashboard.dueBalance.toLocaleString()}</span> — please clear your dues
//             </p>
//             <button onClick={() => router.push("/customer/payment")} className="text-xs font-medium px-4 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg ml-4 whitespace-nowrap transition-colors">
//               Pay now
//             </button>
//           </div>
//         )}
...
//                       <td className="py-2.5 pr-4 text-gray-600 dark:text-gray-300 whitespace-nowrap">{new Date(p.createdAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</td>
...
//   return (
//     <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
//       <Header profile={profile} initials={initials} isVerified={isVerified} onLogout={logout} />
//       <main className="max-w-4xl mx-auto px-6 py-6 space-y-5">
